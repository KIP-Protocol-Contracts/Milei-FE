...
export function DogsPage() {
  return (
    <div className="flex flex-col h-screen">
      <div className="flex flex-col items-center border h-full justify-center p-4">
        <div className="relative mb-6 md:mb-10 text-center">
          <h1 className="font-bold text-2xl md:text-3xl">
            Pregúntele al Consejo Económico Canino
          </h1>
          <div className="absolute right-0 mt-2 w-[200px] md:w-[290px]">
            <Strips />
          </div>
        </div>

        <div className="flex flex-wrap items-center justify-center gap-4 md:gap-20 p-4">
          {dogsList.map((dog, idx) => (
            <div className="flex flex-col items-center gap-4" key={idx}>
              <div className="flex flex-col items-center border-4 border-blue_1 shadow-lg shadow-mil_orange p-4">
                <p className="font-semibold text-xl md:text-3xl mb-4">
                  {dog.name}
                </p>
                <Image
                  src={dog.image}
        ...
        {/* Link ke Halaman Profesor Milai */}
        <Link
          href={'/'}
          className="text-blue_1 font-semibold text-sm mt-6 text-center hover:underline"
        >
